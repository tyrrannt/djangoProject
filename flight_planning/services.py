...
def get_pilot_periodic_check_status(
    ...
    for ct in applicable_check_types:
        ...
        if not rec:
            ...
            details.append({
                ...
                'document_number': '',
                'issued_by': '',
                'message': f"Проверка «{ct.name}» [{ac_name}] не имеет записей о прохождении"
            })
        else:
            ...
            if st == 'expired':
                ...
                msg = f"Проверка «{ct.name}» [{ac_name}] просрочена с {rec.end_date.strftime('%d.%m.%Y')}"
            elif st == 'warning':
                warning_count += 1
                badge_class = 'badge bg-warning text-dark'
                status_label = f"Истекает ({days_left} дн.)"
                msg = f"Проверка «{ct.name}» [{ac_name}] истекает через {days_left} дн. (до {rec.end_date.strftime('%d.%m.%Y')})"
            else:
                valid_count += 1
                badge_class = 'badge bg-success'
                status_label = 'Действует'
                msg = f"Проверка «{ct.name}» [{ac_name}] действует до {rec.end_date.strftime('%d.%m.%Y')}"
    ...
    if has_expired:
        ...
        summary_text = f"Просрочено/отсутствует проверок: {expired_count + missing_count}"
    elif has_warning:
        badge_status = 'warning'
        badge_icon = 'bx-time-five text-warning'
        summary_text = f"Истекает в течение 30 дней проверок: {warning_count}"
    else:
        badge_status = 'ok'
        badge_icon = 'bx-check-circle text-success'
        summary_text = "Все обязательные периодические проверки пройдены и действуют"
