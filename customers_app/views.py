...
from administration_app.utils import boolean_return, get_jsons_data, \
    ...
    get_jsons_data_filter, get_task_title_with_icon, update_1c_physical_person_email
...
@logger.catch
def get_profile_fill(self, context, object):
    profile_info = 0
    user_object = object
...
class AssignCorporateEmailView(LoginRequiredMixin, View):
    """AJAX-контроллер для назначения корпоративной почты сотруднику и синхронизации с 1С (ЗУП).

    Предоставляет защищенный интерфейс для сотрудников ИТ-отдела (группы 'password',
    'Администраторы почты', 'ИТ', 'IT', суперпользователи) для прямого назначения рабочего email.
    Выполняет:
    1. Проверку прав доступа пользователя к управлению учетными записями.
    2. Валидацию формата адреса электронной почты.
    3. Запись email в профиль сотрудника DataBaseUser на корпоративном портале.
    4. Синхронную отправку PATCH-запроса в 1С OData (Catalog_ФизическиеЛица) по person_ref_key.
    5. Опциональную установку рабочего пароля и отправку письма с учетными данными.

    Attributes:
        allowed_groups (List[str]): Список наименований групп пользователей с правом назначения почты.
    """

    allowed_groups: List[str] = ["password", "Администраторы почты", "ИТ", "IT"]

    def _has_permission(self, request) -> bool:
        """Проверяет наличие прав на назначение корпоративной почты.
    ...
    def post(self, request, pk: int, *args, **kwargs) -> JsonResponse:
        """Обрабатывает POST-запрос на назначение email и синхронизацию с 1С.
        ...
        if not self._has_permission(request):
            logger.warning(
                f"Пользователь {request.user} без прав попытался назначить email сотруднику id={pk}"
            )
            return JsonResponse(
                {
                    "status": "error",
                    "message": "У вас нет прав для назначения корпоративной почты сотрудникам.",
                },
                status=403,
            )

        user_obj = get_object_or_404(DataBaseUser, pk=pk)

        # Поддержка как JSON-пейлоада, так и стандартного FormData
        try:
            ...
            return JsonResponse(
                {"status": "error", "message": f"Некорректный формат данных: {str(e)}"},
                status=400,
            )
        ...
        if not email:
            return JsonResponse(
                {"status": "error", "message": "Адрес электронной почты не может быть пустым."},
                status=400,
            )
        ...
        try:
            ...
            return JsonResponse(
                {"status": "error", "message": f"Некорректный формат email: '{email}'."},
                status=400,
            )

        # 1. Синхронизация с 1С (если флаг включен и у пользователя есть person_ref_key)
        sync_1c_success = False
        sync_1c_message = ""
        if sync_1c:
            person_key = user_obj.person_ref_key
            if person_key and person_key not in ["", "00000000-0000-0000-0000-000000000000"]:
                sync_1c_success, sync_1c_message = update_1c_physical_person_email(
                    person_ref_key=person_key, email=email, base_index=0
                )
                if not sync_1c_success:
                    logger.warning(
                        f"Ошибка синхронизации email в 1С для пользователя {user_obj.username}: {sync_1c_message}"
                    )
            else:
                sync_1c_message = (
                    "GUID физлица в 1С отсутствует. Email сохранен только локально на портале."
                )

        # 2. Сохранение email в Django
        ...
        if change_password and work_password:
            ...
            if hasattr(user_obj, "user_work_profile") and user_obj.user_work_profile:
                ...
                user_obj.user_work_profile.save(update_fields=["work_email_password"])
        ...
        if send_notification and email and work_password and change_password:
            try:
                ...
                logger.info(
                    f"Задача отправки уведомления с учетными данными для {email} поставлена в очередь."
                )
            except Exception as ex:
                logger.error(f"Ошибка постановки задачи отправки уведомления: {str(ex)}")

        # Формирование итогового сообщения
        if sync_1c and sync_1c_success:
            final_message = (
                f"Корпоративная почта {email} успешно назначена и синхронизирована с 1С (ЗУП)!"
            )
        elif sync_1c and not sync_1c_success:
            final_message = f"Почта {email} сохранена на портале, но при синхронизации с 1С возникло замечание: {sync_1c_message}"
        else:
