...
class OnlineUsersConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        if self.scope["user"] == AnonymousUser():
            ...
            if user.is_authenticated:  # Проверяем, авторизован ли пользователь
                self.online_users.add(user)  # Добавляем имя пользователя в множество online_users
    ...
    async def disconnect(self, close_code):
        ...
        user = self.scope['user']  # Получаем объект пользователя из scope
        if user.is_authenticated:  # Проверяем, авторизован ли пользователь
            self.online_users.discard(user)  # Удаляем имя пользователя из множества online_users
